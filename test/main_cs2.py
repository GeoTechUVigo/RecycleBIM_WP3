...
# Packages 
from src.storey_seg.pointcloudpy import PointCloudPy
from src.storey_seg.storeydivisionpy import StoreyDivision
from src.utils import functionspy
...
pcd_obj = PointCloudPy(pcd=pcd_cloud, traj=pcd_traj,
                             col_pcd=['x','y','z', 'time'], 
                             col_traj=['x','y','z', 'time'])


# Instance of HistDivision
storey_div_obj = StoreyDivision(pcd_obj.trajectory, pcd_obj.pcd, traj_bin, pcd_bin, dss)

# Process
storey_div_obj.process(min_hor_dist, n_max)

# Calculate storey time intervals
times_floor_dict = storey_div_obj.get_floors_intervals()

# Sorey segmentation in input pointcloud data
pcd_obj.time_segmentation(time_seg=times_floor_dict, name='id_storey', traj=pcd_traj)

# Identifier of floors
ids_floors = storey_div_obj.id_storey
logging.info('Floor segmentation has been successfully performed')
    

# Each floor is analysed separately
for id_floor in ids_floors:
    pcd_df = pcd_obj.pcd    
    
    out_filename = folder_cloud + "_storey_" + str(id_floor) + ".txt"
    out_filepath = folder_results + folder_cloud + "\\" + out_filename

    # Write segmented pcd data in txt file
    pcd_df[pcd_df.id_storey == id_floor].to_csv(out_filepath, sep=' ', 
                                      columns=['x', 'y', 'z', 'time', 'id_storey'],
                                      index=False) 


end_time = time.time()

# Calculate the time difference to get the execution time
execution_time = end_time - ini_time

# Save the execution time in seconds of storey segmentatation
with open(folder_results + folder_cloud + '\\time_seg_storeys.txt', 'w') as archivo:
    archivo.write(f"The program takes {execution_time} seconds to execute.")


# Now start with room segmentation
name_cloud = "\\cs2_storey_1 - Cloud.txt"
ini_time = time.time()
