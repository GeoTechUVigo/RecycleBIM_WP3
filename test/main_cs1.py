...
path = input('The path where the data is saved is: ')
folder_cloud = "\\cs1"
folder_results = input('The path where the results will be saved is: ')
name_cloud = "\\cs1_storey_0_002.txt"
name_f = "_pointcloud_0_02.txt"
...
# Packages 
from parameters import parameters_cs1 as pm
...
from src.elements_seg import doors_v2
from src.elements_seg import walls_index
from src.adjacency import intersection_points_index
from src.adjacency import points_room_order
...
with open(folder_results + folder_cloud + '\\time_morph_seg' + name_f, 'w') as file:
    ...
    floor, obstacles, ceiling, idx_floor, idx_obs, idx_ceil = efc.extract_floor_ceiling(pcd_down_clean, pm.nt,
                                                                                        search_param=o3d.geometry.KDTreeSearchParamKNN(40), 
                                                                                        ransac_th=pm.ransac_th, return_index=True, timestamp=None, 
                                                                                        visualize=False)
    ...
    boundary_pol, floor_plane, ceiling_plane, pcd_in, clean_idx = rop.remove_no_continuous(pcd_in, floor, obstacles, ceiling, idx_floor, idx_obs,
                                                                                       idx_ceil, pm.eps, pm.pt, pm.alpha, pm.voxel_size, pm.zmax, pm.zmin, 
                                                                                       pm.dist, vox_idx, idx_clean, visualize = False, save = False)    
    ...
    image_erode, se_erosion = erosion.morphological_erosion(labelled_vox_df, pm.se, 
                                                pm.width_door, pm.empty_in_lbl, 
                                                pm.voxel_size)
    ...
    # Individualisation
    labels_out = individualisation.room_individualisation(image_erode, pm.voxel_size)
    ...
    room_lab_arr, segmented_3d_image = ovc.occupied_voxels_classification(labelled_vox_df, vxl_idx_arr,
                                                                                dilated_image, rng_lst, pm.occ_lbl)
    ...
    floor, obstacles, ceiling, idx_floor, idx_obs, idx_ceil = efc.extract_floor_ceiling(array_to_o3d.array_to_o3d_point_cloud(class_pcd_df.loc[:, 'x':'z'].values), pm.nt,
                                                                                        search_param=o3d.geometry.KDTreeSearchParamKNN(40), 
                                                                                        ransac_th=pm.ransac_th, return_index=True, timestamp=None, 
                                                                                        visualize=False)
    ...
    # Doors
    bbx_door_lst, bbx_door_pts_lst, adj_room_lst = doors_v2.door_detection(pm.vox_labels,
                                                                        pm.voxel_size, voxel_grid, 
                                                                        pm.empty_in_lbl, pm.occ_lbl, 
                                                                        pm.pts_door, pm.eps_door, 
                                                                        pm.min_pt_door, 
...
#%%    
    # Walls
    # Create a folder for walls
    path_walls = folder_results + folder_cloud + "\\walls"
    try:
        os.mkdir(path_walls)
    ...
    # The RANSAC algorithm is applied for wall detection
    equation_df_walls_t, df_walls_t, lines_position_t, z_0, walls_df, class_pcd_df = walls_index.walls(class_pcd_df, pm.ex_v, pm.max_distance,
                                                                                                       pm.threshold, pm.init_n, pm.iterat, pm.h, pm.dist_pl, 
                                                                                                       pm.min_angle, pm.max_angle, pm.dist_centroids_1, path_walls)
        
    # The intersection points of the walls are calculated
    points_intersection, room_data_list = intersection_points_index.intersect(class_pcd_df, lines_position_t, df_walls_t, pm.b_f, pm.min_p_int, 
                                                                              pm.max_dist_int, pm.min_angle_d, pm.max_angle_d, pm.perfect_angle,
                                                                              pm.flat_angle, pm.ex_v)
    
    with open(path_walls + "\\info_data.json", "w") as json_file:
        json.dump(room_data_list, json_file, indent=4)    
    # The points are ordered
    points_f = points_room_order.points_room_order(points_intersection, path_walls)
    points_tuples = [[tuple(point) for point in sublist] for sublist in points_f]
      
    # The orthogonal projection on the floor and ceiling planes is calculated
    modified_points_intersection = intersection_points_index.only_intersection_points(points_tuples, z_0)   
    list_points_ceiling, list_points_floor = intersection_points_index.floor_ceiling_plane_json(class_pcd_df, modified_points_intersection, pm.min_ratio, 
                                                                                                pm.threshold, pm.iterat)
    # Save information
    with open(path_walls + "\\info_data.json", "r") as file:
        room_json = json.load(file)
        
    for room, points_floor, points_ceiling in zip(room_json, list_points_floor, list_points_ceiling):
        room["floor_projection"] = [np.round(point, 2).tolist() for point in points_floor]
        room["ceiling_projection"] = [np.round(point, 2).tolist() for point in points_ceiling]

    with open(path_walls + "\\info_data.json", "w") as file:
        json.dump(room_json, file, indent=4)
    
    np.savetxt(path_walls + '\\list_point_floor.txt', np.concatenate(list_points_floor))      
    np.savetxt(path_walls + '\\list_point_ceiling.txt', np.concatenate(list_points_ceiling))       
        
    # Replace NaN in 'identity'
    class_pcd_df['id_entity'].fillna(99, inplace=True)
    class_pcd_df['id_entity'] = class_pcd_df['id_entity'].astype(int)
    class_pcd_df.to_csv(folder_results + folder_cloud + "\\seg_with_walls" + name_f, sep=' ', header=True, index=False)           
...
if pm.write == True:
    ...
    erode_image_df.to_csv(folder_results + folder_cloud + '\\erode_im.csv', sep = ",", index=False) 
