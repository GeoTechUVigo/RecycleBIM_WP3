...
folder_cloud = "\\cs3"
name_cloud = "\\Initial_cs3_005.txt"
...
name_cloud = "\\cs3_storey_0.txt"
name_f = "_pointcloud_0_01.txt"
# Save start time
ini_time = time.time()

# Open a file in write mode ('w')
with open(folder_results + folder_cloud + '\\time_morph_seg' + name_f + ".txt", 'w') as file:
...
# Dataframe
class_pcd_df.to_csv(folder_results + folder_cloud + "\\seg" + name_f + ".txt", sep=' ', header=True, index=False)


# If we see that a point cloud has been calculated incorrectly then we change 
# the parameters and execute
while True:
    ...
    if answer.lower() == 'yes':
        ...
        nt = 0.9
        alpha = 3.2
        ransac_th = float(input("Ransac that we want to modify: ")) #0.5
        se = None
        vox_idx = np.array([])
        clean_idx = np.array([])
        vox_labels = pd.DataFrame() 
        width_door = float(input("Width door that we want to modify: ")) #1.3
        dist = float(input("Dist that we want to modify: ")) #0.01
        class_selected_rows = mix_room.mix_room(pcd, n_room, nt, ransac_th, eps,
                                                pt, voxel_size, alpha, zmax, 
                                                zmin, dist, vox_labels, se,
                                                width_door, empty_in_lbl,
                                                occ_lbl)
        ...
        m = pcd_r['id_room'].max()
        for i in range(min(class_selected_rows['id_room'].unique()), 
                       max(class_selected_rows['id_room'].unique())+1):
            class_selected_rows.loc[class_selected_rows['id_room'] == i, 'id_room'] = m + i

        class_pcd_df = pd.concat([pcd_r, class_selected_rows], ignore_index=True)
