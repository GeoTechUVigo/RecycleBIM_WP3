...
def mix_room(pcd, n_room, nt, ransac_th, eps, pt, voxel_size, alpha, zmax, zmin,
             dist, vox_labels, se, empty_in_lbl, occ_lbl,width_door):
    """
    ...
    [boundary_pol, floor_plane, ceiling_plane, selected_rows, clean_idx
     ] = rop.remove_no_continuous(selected_rows, floor, obstacles, ceiling, 
    ...
    class_selected_rows = pd.DataFrame(np.hstack((selected_rows.loc[idx_in_pcd, 'x':'id_storey'].values,
