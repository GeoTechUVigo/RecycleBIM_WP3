...
def remove_no_continuous(pcd_in, floor, obstacles, ceiling, idx_floor, idx_obs,
    ...
    """
    # Remove non continuos celling
    floor_clean, idx_clean_floor = clean_exteriors.clean_exteriors(
        floor, epsilon = eps, points_min = pt, return_index=(True))
    
    # Compute concave hull from cleaned celling
    concave_hull = alph.alphashape(np.asarray(floor_clean.points)[:,0:2], alpha)
    boundary_pol = concave_hull.buffer(voxel_size*2)
    # Extract cleaned floor by using concave hull
    ceil_clean, idx_clean_ceil =  bounding_polygon_filter.bounding_polygon_filter(
        floor, boundary_pol, zmax, zmin, eliminate='outside')
    ceil_clean = floor.select_by_index(idx_clean_ceil)
    # Extract cleaned obstacles by using concave hull
    obst_clean, idx_clean_obst =  bounding_polygon_filter.bounding_polygon_filter(
        obstacles, boundary_pol, zmax, zmin, eliminate='outside')
    obst_clean = obstacles.select_by_index(idx_clean_obst)
    
    if visualize:        
        o3d.visualization.draw_geometries(
            [floor_clean, ceil_clean, obst_clean], 
    ...
    floor_plane, _ = floor_clean.segment_plane(distance_threshold=dist, 
                                               ransac_n=init_n, 
                                               num_iterations=zmax)
    ceiling_plane, _ = ceil_clean.segment_plane(distance_threshold=dist, 
                                                ransac_n=init_n,
                                                num_iterations=zmax)
