...
def remove_no_continuous(pcd_in, floor, obstacles, ceiling, idx_floor, idx_obs,
                         ...
                         dist, init_n, vox_idx, idx_clean, visualize, save):
    """
    ...
    dist : float
        Maximum distance a point can have to an estimated plane to be considered 
        an inlier.
    init_n: int
    ...
    boundary_pol : shapely.geometry.polygon.Polygon
        Outline of the ceiling of the storey.
    ...
    """
    # Remove non continuos celling
    ceiling_clean, idx_clean_ceil = clean_exteriors.clean_exteriors(
        ceiling, epsilon = eps, points_min = pt, return_index=(True))
    
    # Compute concave hull from cleaned celling
    concave_hull = alph.alphashape(np.asarray(ceiling_clean.points)[:,0:2], 
                                   alpha)
    boundary_pol = concave_hull.buffer(voxel_size*2)
    # Extract cleaned floor by using concave hull
    [floor_clean, idx_clean_floor
     ] =  bounding_polygon_filter.bounding_polygon_filter(floor, boundary_pol, 
                                                          zmax, zmin, 
                                                          eliminate='outside')
    floor_clean = floor.select_by_index(idx_clean_floor)
    # Extract cleaned obstacles by using concave hull
    [obst_clean, idx_clean_obst
     ] =  bounding_polygon_filter.bounding_polygon_filter(obstacles, 
                                                          boundary_pol, zmax, 
                                                          zmin, 
                                                          eliminate='outside')
    ...
    if visualize:        
        o3d.visualization.draw_geometries([
            floor_clean, ceiling_clean, obst_clean], 
    ...
    floor_plane, _ = floor_clean.segment_plane(distance_threshold=dist, 
                                               ransac_n=init_n, 
                                               num_iterations=zmax)
    ceiling_plane, _ = ceiling_clean.segment_plane(distance_threshold=dist, 
                                                   ransac_n=init_n, 
                                                   num_iterations=zmax)
    ...
    glob_idx_filt_obs = vox_idx[idx_clean][idx_obs][idx_clean_obst][
        idx_floor_plane][idx_ceil_plane]
    
    clean_idx = np.concatenate((glob_idx_ceil_clean, glob_idx_floor_clean, 
                                glob_idx_filt_obs))
        
    if visualize or save:
        filt_floor_o3d = array_to_o3d.array_to_o3d_point_cloud(
            pcd_in.loc[glob_idx_floor_clean, 'x':'z'].values)
        filt_ceiling_o3d = array_to_o3d.array_to_o3d_point_cloud(
            pcd_in.loc[glob_idx_ceil_clean, 'x':'z'].values)
        filt_obstacles_o3d = array_to_o3d.array_to_o3d_point_cloud(
            pcd_in.loc[glob_idx_filt_obs, 'x':'z'].values)
        ...
        if visualize:
            ...
            o3d.visualization.draw_geometries(
                [filt_floor_o3d, filt_ceiling_o3d, filt_obstacles_o3d], 
                window_name="Cleaned pcd")
