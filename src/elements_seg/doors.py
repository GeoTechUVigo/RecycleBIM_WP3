...
def door_detection(vox_labels, voxel_size, voxel_grid, empty_in_lbl, 
    ...
    # Filtar por normales sirve para identificar objetos verticales en la direccion de Z o para separar
    # la nube de puntos en segmentos horizontales para analizar diferentes niveles en una escena.
    # Aumentar el valor de Z sería más restrictivo y seleccionaría solo puntos con normales más verticalmente orientadas.
    # Disminuir su valor sería menos restrictivo y permitiría una gama más amplia de orientaciones verticales
    ...
    if visualize:        
        #np.asarray(unrecovered_vxl_o3d.colors)[idx_nor_discard, :] = [0., 1., 0.]
    ...
    for lbl_cand in range(max_label+1):    
        ...
        # min_axis ontendría el índice del eje (0 para x, 1 para y, 2 para z) 
        # que tiene la menor variación en términos de longitud en el cuadro 
        # delimitador alineado con los ejes del candidato de la nube de puntos
        ...
        if min_axis != 2:
            ...
            uni_adj, count_adj = np.unique(adj_rooms, return_counts=True) # returns the unique elements and frequencies
            #print(uni_adj, count_adj)
            #print(len(uni_adj))
                   
            if (len(idx_occ) > len(cand_pcd.points)) or len(uni_adj) != 2: # or len(idx_out) == 0:
                #print("{} discarded".format(lbl_cand))
