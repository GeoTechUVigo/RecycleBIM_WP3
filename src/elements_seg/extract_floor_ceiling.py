...
def extract_floor_ceiling(cloud, nt, search_param, ransac_th, timestamp,
    ...
    _ = cloudPC.add_scalar_field("plane_fit", max_dist=ransac_th, max_iterations=100, n_inliers_to_stop=None)
    ...
    ceiling = pc.PyntCloud(pd.DataFrame(plane, index=None, columns=['x', 'y', 'z']))
    ...
    notCeiling = pc.PyntCloud(pd.DataFrame(rest, index=None, columns=['x', 'y', 'z']))
    notCeiling.add_scalar_field("plane_fit", max_dist=ransac_th, max_iterations=100, n_inliers_to_stop=None)
    ...
    floor = pc.PyntCloud(pd.DataFrame(notCeiling.xyz[is_plane==1], index=None, columns=['x', 'y', 'z']))
    ...
    obs_idx = np.arange(len(cloud.points))[~np.isin(np.arange(len(cloud.points)),planes_idx)]
    ...
    obstacle_walls = pc.PyntCloud(pd.DataFrame(notCeiling.xyz[is_plane==0], index=None, columns=['x', 'y', 'z']))
    ...
    if timestamp is None:
        ...
        if not return_index:
            return floor, floor_timestamp, obstacle_walls, obst_timestamp, ceiling, ceiling_timestamp
        else:
            return floor, floor_timestamp, obstacle_walls, obst_timestamp, ceiling, ceiling_timestamp, plane2, obs_idx, plane1